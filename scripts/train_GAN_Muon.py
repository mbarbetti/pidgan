...
parser = argparser_training(model="Muon", description="MuonGAN training setup")
...
npzfile = np.load(f"{data_dir}/Muon-{args.particle}-{args.data_sample}-trainset.npz")
...
d_lr_sched = LearnRateExpDecay(
    ...
    decay_steps=hp.get("d_decay_steps", 150_000),
...
with open(
    f"{models_dir}/Muon_{args.particle}_models/tX_{args.data_sample}.pkl", "rb"
) as file:
...
with open(
    f"{models_dir}/Muon_{args.particle}_models/tY_{args.data_sample}.pkl", "rb"
) as file:
...
model_name += f"_Muon_{args.particle}_{args.data_sample}_gan"

export_model_dirname = f"{models_dir}/Muon_{args.particle}_models/{model_name}"
export_img_dirname = f"{images_dir}/Muon_{args.particle}_images/{model_name}"
...
report = Report()
report.add_markdown('<h1 align="center">MuonGAN training report</h1>')
